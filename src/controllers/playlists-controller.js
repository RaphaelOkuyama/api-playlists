...
module.exports = {
  ...
  // Função para pegar uma música pelo ID dentro de uma playlist
  getMusicById: (req, res) => {
  const { playlistId, musicId } = req.params;  // Obtém os IDs da playlist e da música

  // Encontra a playlist pelo ID
  const playlist = playlists.find(pl => pl.id === +playlistId);
  ...
  // Encontra a música pelo ID dentro da playlist
